 ...
 * Instead of the TCP layer sending packets point-to-point to each other member,
 * it sends the packet to the router which - depending on the target address -
 * multicasts or unicasts it to the group / or single member.
 * <p/>
 ...
 * loaded from. Therefore, an applet would create a normal channel plus protocol
 * stack, but the bottom layer would have to be the TCP layer which sends all
 * packets point-to-point (over a TCP connection) to the router, which in turn
 * forwards them to their end location(s) (also over TCP). A centralized router
 * would therefore have to be running on the host the applet was loaded from.
 * <p/>
 ...
 * <p/>
 * Note that a GossipRouter is also a good way of running JGroups in Amazon's
 * EC2 environment which (as of summer 09) doesn't support IP multicasting.
 * 
 * @author Bela Ban
 ...
 * @since 2.1.1
 */
public class RMGossipRouter {
	public static final byte CONNECT = 1; // CONNECT(group, addr) --> local
											// address
	public static final byte DISCONNECT = 2; // DISCONNECT(group, addr)
	public static final byte GOSSIP_GET = 4; // GET(group) --> List<addr>
												// (members)
	...
	/**
	 * Store physical address(es) associated with a logical address. Used mainly
	 * by TCPGOSSIP
	 */
	...
	public RMGossipRouter() {
		this(PORT);
	}

	public RMGossipRouter(int port) {
		this(port, null);
	}

	public RMGossipRouter(int port, String bindAddressString) {
		this(port, bindAddressString, false, 0);
	}

	public RMGossipRouter(int port, String bindAddressString, boolean jmx) {
		this(port, bindAddressString, jmx, 0);
	}

	public RMGossipRouter(int port, String bindAddressString, boolean jmx, long expiryTime) {
		this.port = port;
	...
	/**
	 * Lifecycle operation. Called after create(). When this method is called,
	 * the managed attributes have already been set.<br>
	...
	private static void sendToMember(Address dest, final DataOutputStream out, byte[] msg) throws Exception {
		if (out == null)
			return;
		synchronized (out) {
			GossipData request = new GossipData(RMGossipRouter.MESSAGE, null, dest, msg);
			request.writeTo(out);
			out.flush();
		}
	}
					...
					for (Iterator<Entry<Address, ConnectionHandler>> i = map.entrySet().iterator(); i
						...
						try {
							for (Address a : ch.logical_addrs) {
								GossipData suspect = new GossipData(RMGossipRouter.SUSPECT);
	...
	class ConnectionHandler implements Runnable {
		...
		private void readLoop() {
			while (isRunning()) {
				...
				try {
					...
					case RMGossipRouter.CONNECT:
						handleConnect(request, addr, group);
						break;

					case RMGossipRouter.PING:
						// do nothing here - client doesn't expect response data
						break;

					case RMGossipRouter.MESSAGE:
						if (request.getBuffer() == null || request.getBuffer().length == 0) {
							if (log.isWarnEnabled())
					...
					case RMGossipRouter.GOSSIP_GET:
					...
					case RMGossipRouter.DISCONNECT:
						try {
		...
		private void handleConnect(GossipData request, Address addr, String group) throws Exception {
			...
			try {
				...
				// group name, logical address, logical name, physical addresses
				// (could be null)
				logical_addrs.add(addr); // allows us to remove the entries for
											// this connection on
											// socket close
				...
				if (map == null) {
					...
					routingTable.put(group, map); // no concurrent requests on
													// the same connection
				}
				map.put(addr, this);
